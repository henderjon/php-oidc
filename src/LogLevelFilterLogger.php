 ...
 *                Nothing passes. `$levels` is empty and `$allow` defaults true, so no level
 ...
 *
 *   all          new LogLevelFilterLogger($logger, [], allow: false)
 ...
 *
 *   all, except  new LogLevelFilterLogger($logger, [ LogLevel::DEBUG ], allow: false)
 ...
 *
 * The default, `$allow: true`, fails closed - a level nobody named gets dropped, including
 * one this library or a wrapped logger adds later. `$allow: false` fails open - a level
 ...
 * `$excludedLevels` - it holds an allow-list when `$allow` is true and a deny-list when it is
...
final class LogLevelFilterLogger extends AbstractLogger {
	 ...
	 *                             level.
	 * @param bool $allow When true (the default), $levels is an allow-list: only a level
	 ...
	 * deny-list - `new LogLevelFilterLogger($logger, [], allow: false)` - since "everything
	...
	public static function all( LoggerInterface $logger ): self {
		return new self($logger, [], allow: false);
	}

	public function log( $level, string|\Stringable $message, array $context = [] ): void {
		if( in_array($level, $this->levels, true) !== $this->allow ) {
			return;
		}
