 ...
 *                Nothing passes. `$levels` is empty and `$mode` defaults to `AllowList`, so no
 *                level can ever satisfy the membership test.
 *
 ...
 *
 *   all          new LogLevelFilterLogger($logger, [], LogLevelFilterMode::DenyList)
 ...
 *
 *   all, except  new LogLevelFilterLogger($logger, [ LogLevel::DEBUG ], LogLevelFilterMode::DenyList)
 ...
 *
 * `$levels` is a discrete set, not a range, in either mode - any combination is valid, including
 * combinations no linear severity ordering could express together (`debug` and `critical` alone,
 * with everything between them excluded). This class never validates a level against PSR-3's own
 ...
 *
 * The default, `LogLevelFilterMode::AllowList`, fails closed - a level nobody named gets
 * dropped, including one this library or a wrapped logger adds later. `DenyList` fails open - a
 * level nobody named gets forwarded instead. That is a deliberate trade for the specific case it
 ...
 * `$excludedLevels` - it holds an allow-list under `AllowList` and a deny-list under `DenyList`,
 * and a name tied to one mode would be wrong under the other.
 */
final class LogLevelFilterLogger extends AbstractLogger {
	...
	public function __construct(
		...
		private readonly LogLevelFilterMode $mode = LogLevelFilterMode::AllowList,
	) {
	}
	 ...
	 * deny-list - `new LogLevelFilterLogger($logger, [], LogLevelFilterMode::DenyList)` - since
	 * "everything is forwarded because nothing is excluded" is not obviously what that means at
	 * a glance.
	 */
