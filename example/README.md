...
- `app-tls-policy.php` uses `CurlHttpFetcher` directly (the only example that does - every other one fakes `HttpFetcherInterface`) to show that TLS verification is never something a normal request can opt out of, and that `disableTlsVerificationForLocalDevelopmentOnly` logs a critical diagnostic on every single request while it is active, not just once. It cannot demonstrate curl actually rejecting a bad certificate - this repository's dependencies do not include a test transport capable of serving an invalid/self-signed HTTPS endpoint.
