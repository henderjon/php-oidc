...
- `app-response-limits.php` shows `CurlHttpFetcher`'s `maxResponseBytes` cap accepting a response under the limit and aborting one over it, with the warning it logs when that happens - and, through the full client, a discovery response with an unexpected `Content-Type` (`text/html` instead of JSON) being rejected before its body is ever parsed.
